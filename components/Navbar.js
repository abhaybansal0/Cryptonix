        ...
        if (session) {
            return (
                <>
                    ...
                    <div id="dropdown" className={`z-10 ${dropdownstate ? 'block' : 'hidden'} absolute  bg-slate-100 shadow mt-3 p-3 rounded-lg `}>
                        <ul className='py-2 text-sm text-gray-700 dark:text-gray-400 flex flex-col gap-2'>
                            <Link href="/passwords">
                                <li className='text-lg text-black font-normla hover:bg-gray-300 text-center rounded-lg'>Passwords</li>
                            </Link>
                            <li>

                                <button className='btn flex items-center justify-between gap-2' onClick={() => signOut()}>Log Out
